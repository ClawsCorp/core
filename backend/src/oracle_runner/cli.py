...
def build_parser() -> argparse.ArgumentParser:
    ...
    sync_platform_capital = subparsers.add_parser(
        "sync-platform-capital",
        help="Sync observed USDC FundingPool deposits into append-only platform capital events (oracle HMAC protected).",
    )
    sync_platform_capital.add_argument("--json", action="store_true", help="Print machine-readable JSON output to stdout.")

    reconcile_platform_capital = subparsers.add_parser(
        "reconcile-platform-capital",
        help="Run platform capital on-chain vs ledger reconciliation.",
    )
    reconcile_platform_capital.add_argument("--json", action="store_true", help="Print machine-readable JSON output to stdout.")
    ...
    emit_social_signal = subparsers.add_parser(
        "emit-social-signal",
        help="Append a verified social reputation signal (oracle HMAC protected).",
    )
    emit_social_signal.add_argument("--agent-id", required=True)
    emit_social_signal.add_argument("--platform", required=True)
    emit_social_signal.add_argument("--signal-url")
    ...
    emit_customer_referral.add_argument("--agent-id", required=True)
    emit_customer_referral.add_argument("--referral-id", required=True)
    emit_customer_referral.add_argument("--stage", required=True, choices=["verified_lead", "paid_conversion"])
...
def run(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "sync-platform-capital":
            data = _post_action(client, "/api/v1/oracle/platform-capital-events/sync", b"")
            if json_mode:
                _print_json(data)
            else:
                _print_fields(data, ["transfers_seen", "capital_events_inserted", "marketing_fee_events_inserted"])
            return 0

        if args.command == "reconcile-platform-capital":
            data = _post_action(client, "/api/v1/oracle/platform-capital/reconciliation", b"")
            if json_mode:
                _print_json(data)
            else:
                _print_fields(
                    data,
                    ["ready", "blocked_reason", "delta_micro_usdc", "onchain_balance_micro_usdc", "ledger_balance_micro_usdc", "computed_at"],
                )
            return 0
        ...
        if args.command == "emit-social-signal":
            ...
            if not payload["agent_id"]:
                raise OracleRunnerError("--agent-id is required.")
            ...
            if not payload.get("idempotency_key"):
                ...
                payload["idempotency_key"] = _derive_idempotency_key("social_signal", derived)
        ...
        if args.command == "emit-customer-referral":
            payload = {
                "agent_id": str(args.agent_id).strip(),
                "referral_id": str(args.referral_id).strip(),
            ...
            if not payload["referral_id"]:
                raise OracleRunnerError("--referral-id is required.")
            if not payload.get("idempotency_key"):
                derived = dict(payload)
                derived.pop("idempotency_key", None)
                payload["idempotency_key"] = _derive_idempotency_key("customer_referral", derived)
            body_bytes = json.dumps(payload, separators=(",", ":"), ensure_ascii=True, sort_keys=True).encode("utf-8")
            data = _post_action(
                client,
