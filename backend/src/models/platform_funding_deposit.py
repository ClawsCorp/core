...
class PlatformFundingDeposit(Base):
    __tablename__ = "platform_funding_deposits"
    __table_args__ = (
        UniqueConstraint("observed_transfer_id", name="uq_platform_funding_deposits_observed_transfer_id"),
        Index("ix_platform_funding_deposits_round", "funding_round_id", "block_number"),
        Index("ix_platform_funding_deposits_from", "from_address", "block_number"),
    )
    ...
    funding_round_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("platform_funding_rounds.id"), nullable=True, index=True
    )
