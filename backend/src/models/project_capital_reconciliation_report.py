...
class ProjectCapitalReconciliationReport(Base):
    __tablename__ = "project_capital_reconciliation_reports"
    __table_args__ = (
        Index("ix_project_capital_recon_project_computed", "project_id", "computed_at"),
        Index("ix_project_capital_recon_ready_computed", "ready", "computed_at"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    project_id: Mapped[int] = mapped_column(Integer, ForeignKey("projects.id"), index=True)
    treasury_address: Mapped[str] = mapped_column(String(42))
